...
function ProductDetails() {
  ...
  useEffect(() => {
    ...
    if (product) {
      setStockCount(product.stock || 0);
      // Initialize selectedSize if sizes exist
      if (product.sizes && product.sizes.length > 0) {
        setSelectedSize(product.sizes[0]);
      }
  ...
  const handleAddToCart = async () => {
    ...
    setIsAddingToCart(true);
    try {
      // Pass the product, selectedSize, and variantId to addToCart
      const variantId = selectedVariant ? selectedVariant.variantId : null;
      await addToCart(product, selectedSize, variantId);
      setStockCount((prev) => prev - 1);
      toast.success('Added to cart!');
  ...
  // Helper function to render specification values (handles both strings and nested objects)
  const renderSpecValue = (value) => {
    if (value && typeof value === 'object' && !Array.isArray(value)) {
      // If value is an object, convert it to a string representation
      return Object.entries(value)
        .map(([key, val]) => `${key}: ${val}`)
        .join(', ');
    }
    // If value is a primitive (string, number, etc.), render it directly
    return value || 'N/A';
  };
          ...
          <div className="product-details-section">
            ...
            <div className="details-content">
              ...
              <div className="details-row">
                ...
                <span className="details-value">{product.subcategory || 'N/A'}</span>
              </div>
              <div className="details-row">
                <span className="details-label">Nested Category:</span>
                <span className="details-value">{product.nestedCategory || 'N/A'}</span>
              </div>
              <div className="details-row">
                <span className="details-label">Description:</span>
                <span className="details-value">{product.description || 'N/A'}</span>
              ...
              <div className="details-row">
                <span className="details-label">Offer:</span>
                <span className="details-value">{product.offer || 'N/A'}</span>
              </div>
              <div className="details-row">
                <span className="details-label">Seller:</span>
                <span className="details-value">{product.seller || 'N/A'}</span>
              ...
              <div className="details-row">
                <span className="details-label">Deal Tag:</span>
                <span className="details-value">{product.dealTag || 'N/A'}</span>
              </div>
              <div className="details-row">
                <span className="details-label">Brand:</span>
                <span className="details-value">{product.brand || 'N/A'}</span>
              ...
              <div className="details-row">
                ...
                <span className="details-value">{new Date(product.createdAt).toLocaleDateString()}</span>
              </div>
              {product.specifications && typeof product.specifications === 'object' ? (
                Object.entries(product.specifications).map(([key, value]) => (
                  <div key={key} className="details-row">
                    <span className="details-label">{key}:</span>
                    <span className="details-value">{renderSpecValue(value)}</span>
                  </div>
                ))
              ) : (
                <div className="details-row">
                  <span className="details-label">Specifications:</span>
                  <span className="details-value">N/A</span>
                </div>
              )}
              {selectedVariant && selectedVariant.specifications && typeof selectedVariant.specifications === 'object' ? (
