...
function Checkout() {
  ...
  // Redirect to login if not authenticated, or to products if cart is empty
  useEffect(() => {
    if (!authLoading && !user) {
      navigate('/login');
  ...
  const handleSubmit = async (e) => {
    ...
    const orderData = {
      ...
      shippingAddress: `${formData.fullName}, ${formData.address}`, // Match backend expectation
      payment: {
    ...
    try {
      ...
      console.log('Submitting order - user._id:', user._id, 'token:', token); // Debug
      if (!token) {
        throw new Error('No token found');
      }

      // Submit order to backend with Authorization header
