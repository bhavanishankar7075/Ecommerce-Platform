    ...
    try {
      ...
      if (selectedPaymentMethod === 'cod') {
        const orderData = {
          ...
          payment:  'Cash on Delivery' , // Changed to object
          total,
        };
    ...
    try {
      ...
      if (selectedPaymentMethod === 'cod') {
        ...
        const orderData = {
          userId: user._id,
          items: cart.map(item => ({
            productId: item._id, // Add productId
            name: item.name,
            price: Math.round(Number(item.price) * 100), // Ensure price is in cents
            quantity: item.quantity,
            image: item.image || '',
          })),
          shippingAddress: {
          ...
          total: Math.round(total * 100), // Ensure total is in cents
          paymentMethod: selectedPaymentMethod === 'new' ? 'stripe' : 'saved',
          cardDetails: selectedPaymentMethod === 'new'
            ? {
                cardNumber: formData.cardNumber,
