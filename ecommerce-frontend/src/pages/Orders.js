...
function Orders() {
  ...
  const formatPaymentType = (payment) => {
    if (payment === 'Cash on Delivery') return 'Cash on Delivery';
    if (payment.startsWith('Card ending in')) return 'Online Payment (Card)';
    return payment; // Fallback to the raw value (e.g., "Online Payment")
  };

  const handleExportOrders = () => {
    const orderText = filteredOrders
      .map((order) => `Order #${order._id.slice(-6)} - ${new Date(order.createdAt).toLocaleDateString()} - ₹${order.total.toFixed(2)} - Payment: ${formatPaymentType(order.payment)}`)
      .join('\n');
            ...
            >
              ...
              <div className="order-meta">
                ...
                <span className="order-payment">
                  Payment: {formatPaymentType(order.payment)}
                </span>
