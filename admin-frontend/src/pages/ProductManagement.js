...
function ProductManagement() {
  ...
  const handleSpecificationChange = (section, key, value) => {
    setFormData((prev) => {
  ...
  const handleVariantSpecificationChange = (section, key, value) => {
    setVariantFormData((prev) => {
      const specs = { ...prev.specifications };
      if (!specs[section]) specs[section] = {};
      specs[section][key] = value;
  ...
  const addSpecificationSection = () => {
    ...
    if (sectionName) {
      setFormData((prev) => {
        const specs = { ...prev.specifications };
        if (!specs[sectionName]) specs[sectionName] = {};
        return { ...prev, specifications: specs };
      });
    }
  };
  ...
  const validateVariantForm = () => {
    return true; // Variant images and specs are optional
  };
      ...
      if (variantFormData.newImages && variantFormData.newImages.length > 0) {
        variantFormData.newImages.forEach((image, index) => {
          if (image.file instanceof File) {
            variantForm.append(`variantImages`, image.file); // Use array-like name
          }
        });
      }
              ...
              return (
                <div key={product._id} className="product-card">
                  ...
                  <div className="product-actions">
                    ...
                    <button onClick={() => openVariantModal(product)} className="variant-btn">
                      Add Variant
                    </button>
                  </div>
                </div>
      ...
      {variantModal && (
        <div className="preview-modal">
          <div className="modal-content">
            ...
            <h2>Add Variant for {variantFormData.name}</h2>
            <form onSubmit={handleVariantSubmit} className="product-form">
              <div className="form-group">
