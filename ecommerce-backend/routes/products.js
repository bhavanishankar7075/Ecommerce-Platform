...
router.get('/products', async (req, res) => {
  try {
    const { category, subcategory, nestedCategory } = req.query;
    const query = { isActive: true };

    if (category) query.category = category;
    if (subcategory) query.subcategory = subcategory;
    if (nestedCategory) query.nestedCategory = nestedCategory;
...
router.get('/', verifyAdmin, async (req, res) => {
  try {
    ...
    if (nestedCategory) {
      query.nestedCategory = nestedCategory;
    }
...
router.put('/:id', verifyAdmin, upload.fields([
  ...
  try {
    ...
    if (req.files['variantMainImage'] || req.files['variantImages']) {
      const newVariant = {
        variantId: Date.now().toString(), // Unique ID
        specifications: parsedSpecifications || {},
      };
    ...
    product.nestedCategory = nestedCategory || product.nestedCategory;
