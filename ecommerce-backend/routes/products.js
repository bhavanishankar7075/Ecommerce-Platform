...
const fs = require('fs').promises; // Use fs.promises for all file operations
const Product = require('../models/Product');

// Determine upload directory based on environment (consistent with server.js)
...
// Ensure upload directory exists (asynchronous)
(async () => {
  try {
...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    console.log(`Saving file to: ${uploadDir} - Checking accessibility...`);
    (async () => {
      try {
        await fs.access(uploadDir, fs.constants.R_OK | fs.constants.W_OK);
        console.log(`Directory ${uploadDir} is accessible for saving.`);
        cb(null, uploadDir);
      } catch (err) {
...
const formatProductImage = (product) => {
  ...
  if (product.image) {
    ...
    product.image = 'http://localhost:5002/default-product.jpg'; // Fallback image
  }
  if (product.images && product.images.length > 0) {
    product.images = product.images.map(img => {
      ...
      return 'http://localhost:5002/default-product.jpg'; // Fallback image
    });
  } else {
    product.images = [];
  }
...
router.post('/', verifyAdmin, logFields, upload.fields([
...
router.put('/:id', verifyAdmin, logFields, upload.fields([
  { name: 'mainImage', maxCount: 1 },
  { name: 'additionalImages', maxCount: 10 },
]), async (req, res) => {
  const {
  ...
  try {
    ...
    if (existingImages) {
      ...
      const cleanedImagesList = newImagesList.map(img => {
        if (img.startsWith(process.env.BASE_URL || 'http://localhost:5001')) {
          return img.replace(process.env.BASE_URL || 'http://localhost:5001', '');
        }
        return img;
      });
